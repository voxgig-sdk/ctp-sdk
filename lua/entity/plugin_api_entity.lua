-- Ctp SDK PluginApi entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PluginApiEntity = {}
PluginApiEntity.__index = PluginApiEntity


function PluginApiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PluginApiEntity)
  self._name = "plugin_api"
  self._client = client
...
function PluginApiEntity:get_name()
  return self._name
end


function PluginApiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PluginApiEntity.new(self._client, opts)
end
...
function PluginApiEntity:mark_deleted()
  self._deleted = true
end


function PluginApiEntity:deleted()
  return true == self._deleted
end


function PluginApiEntity:data_set(args)
  if args ~= nil then
...
function PluginApiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PluginApiEntity:match_set(args)
  if args ~= nil then
...
function PluginApiEntity:match_get()
...
function PluginApiEntity:stream(action, args, callopts)
...
---@param reqmatch PluginApiLoadMatch
---@param ctrl? table
---@return PluginApi
---@return string? err
function PluginApiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PluginApiEntity:_run_op(ctx, post_done)
  local utility = self._utility
