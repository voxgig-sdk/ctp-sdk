...
local PluginEntity = {}
PluginEntity.__index = PluginEntity


function PluginEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PluginEntity)
  self._name = "plugin"
  self._client = client
...
function PluginEntity:get_name()
  return self._name
end


function PluginEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PluginEntity.new(self._client, opts)
end
...
function PluginEntity:mark_deleted()
  self._deleted = true
end


function PluginEntity:deleted()
  return true == self._deleted
end


function PluginEntity:data_set(args)
  if args ~= nil then
...
function PluginEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PluginEntity:match_set(args)
  if args ~= nil then
...
function PluginEntity:match_get()
...
function PluginEntity:stream(action, args, callopts)
...
---@param reqmatch PluginLoadMatch
---@param ctrl? table
---@return Plugin
---@return string? err
function PluginEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PluginEntity:_run_op(ctx, post_done)
  local utility = self._utility
