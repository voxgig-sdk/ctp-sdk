-- Ctp SDK JsonApi entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local JsonApiEntity = {}
JsonApiEntity.__index = JsonApiEntity


function JsonApiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JsonApiEntity)
  self._name = "json_api"
  self._client = client
...
function JsonApiEntity:get_name()
  return self._name
end


function JsonApiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JsonApiEntity.new(self._client, opts)
end
...
function JsonApiEntity:mark_deleted()
  self._deleted = true
end


function JsonApiEntity:deleted()
  return true == self._deleted
end


function JsonApiEntity:data_set(args)
  if args ~= nil then
...
function JsonApiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JsonApiEntity:match_set(args)
  if args ~= nil then
...
function JsonApiEntity:match_get()
...
function JsonApiEntity:stream(action, args, callopts)
...
---@param reqmatch JsonApiLoadMatch
---@param ctrl? table
---@return JsonApi
---@return string? err
function JsonApiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function JsonApiEntity:_run_op(ctx, post_done)
  local utility = self._utility
