...
The Lua SDK for the Ctp API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:JsonApi(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Ctp(nil):load(
  { id = "test01" }, nil
)
