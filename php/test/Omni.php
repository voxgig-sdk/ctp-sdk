...
//    only - a typed CtpContext there would make every ctx
...
final class CtpOmni
{
    ...
    public static function err_from_map($m): ?CtpError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CtpError(is_string($code) ? $code : '', $msg);
    }
