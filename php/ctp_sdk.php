...
class CtpSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CtpHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CtpHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CtpFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CtpFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CtpUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CtpHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CtpHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CtpHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CtpError($op . "_allow",
                "CtpSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CtpHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CtpError("graphql_error",
                "CtpSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function JsonApi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_json_api === null) {
                $this->_json_api = new JsonApiEntity($this, null);
            }
            return $this->_json_api;
        }
        return new JsonApiEntity($this, $data);
    }
    ...
    public function PluginApi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_plugin_api === null) {
                $this->_plugin_api = new PluginApiEntity($this, null);
            }
            return $this->_plugin_api;
        }
        return new PluginApiEntity($this, $data);
    }
