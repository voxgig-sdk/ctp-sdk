...
class PluginEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PluginEntity($this->_client, $opts);
    }

    /**
     * @param Plugin|array $args Plugin data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CtpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Plugin|array The current Plugin data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Plugin fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CtpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Plugin fields).
     */
     ...
     *
     * @param PluginLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PluginLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Plugin|array The loaded Plugin as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CtpHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
