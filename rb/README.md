...
The Ruby SDK for the Ctp API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.JsonApi` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = CtpSDK.test(testopts, sdkopts)
```
...
raise a `CtpError` (a `StandardError` subclass), so wrap
...
The main module (`Ctp_sdk`) exports the SDK class
