# Ctp Ruby SDK Reference

Complete API reference for the Ctp Ruby SDK.
...
#### `CtpSDK.test(testopts = nil, sdkopts = nil)`
...
```ruby
result = client.JsonApi.load({ "urn" => "urn" })
```
...
```ruby
result = client.PluginApi.load({ "installplugin" => "installplugin" })
```
