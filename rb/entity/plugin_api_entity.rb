...
class PluginApiEntity
  ...
  def make
    opts = @_entopts.dup
    PluginApiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CtpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PluginApi, Hash] the current PluginApi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CtpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PluginApi fields)
  def match_get
  ...
  #
  # @param reqmatch [PluginApiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PluginApi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PluginApi, Hash] the loaded PluginApi; raises CtpError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CtpHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
