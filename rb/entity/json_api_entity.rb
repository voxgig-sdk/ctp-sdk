...
class JsonApiEntity
  ...
  def make
    opts = @_entopts.dup
    JsonApiEntity.new(@_client, opts)
  end
  ...
  # @return [JsonApi, Hash] the current JsonApi data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of JsonApi fields)
  def match_get
  ...
  #
  # @param reqmatch [JsonApiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.JsonApi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [JsonApi, Hash] the loaded JsonApi; raises CtpError on failure
