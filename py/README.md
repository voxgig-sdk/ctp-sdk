...
The Python SDK for the Ctp API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.JsonApi(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Ctp(None).load(
    {"id": "test01"}, None
)
