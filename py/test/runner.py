...
class CtpTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CtpTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CtpTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CtpTestRunner.getenv("CTP_TEST_LIVE")
        override = CtpTestRunner.getenv("CTP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CtpTestRunner.getenv(key)
        ...
        explain = CtpTestRunner.getenv("CTP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CTP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CtpTestRunner._test_control is not None:
            return CtpTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CtpTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CtpTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CtpTestRunner.load_test_control()
...
def load_env_local():
    CtpTestRunner.load_env_local()


def env_override(m):
    return CtpTestRunner.env_override(m)


def entity_data(v):
    return CtpTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CtpTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CtpTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CtpTestRunner.load_test_control()


def live_client_options():
    return CtpTestRunner.live_client_options()


def live_delay_ms():
    return CtpTestRunner.live_delay_ms()
