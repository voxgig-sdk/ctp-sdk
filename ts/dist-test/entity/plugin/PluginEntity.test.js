...
(0, node_test_1.describe)('PluginEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'plugin.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['plugin01', 'plugin02', 'plugin03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CTP_TEST_PLUGIN_ENTID'];
