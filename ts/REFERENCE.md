# Ctp TypeScript SDK Reference

Complete API reference for the Ctp TypeScript SDK.
...
**Returns:** `CtpSDK` instance in test mode.
...
Create a new `JsonApi` entity instance.
...
**Returns:** `JsonApiEntity` instance.
...
#### `PluginApi(data?: object)`

Create a new `PluginApi` entity instance.
...
**Returns:** `PluginApiEntity` instance.
...
```ts
const result = await client.JsonApi().load({ urn: 'urn' })
```
...
Return the parent `CtpSDK` instance.
...
```ts
const result = await client.PluginApi().load({ installplugin: 'installplugin' })
```
