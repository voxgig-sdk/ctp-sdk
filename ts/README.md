...
The TypeScript SDK for the Ctp API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.JsonApi()` — each with a small set of operations (`load`)
...
```ts
const client = new CtpSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `CtpSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CtpSDK.test(testopts?, sdkopts?)` | `CtpSDK` | Create a test-mode client. |
...
| `client` | `client(): CtpSDK` | Return the parent SDK client. |
