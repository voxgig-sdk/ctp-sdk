...
//     (\`new CtpSDK(...)\` / \`CtpSDK.test(...)\` become
//     \`CtpSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"json_api":{"test01":{"id":"test01"}},"plugin":{"test01":{"id":"test01"}},"plugin_api":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CtpSDK(...)\` and any
// \`CtpSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CtpSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CtpSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
