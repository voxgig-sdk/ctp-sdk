// Command Ctp-cli is a CLI + REPL for the Ctp SDK,
...
const entitiesHelp = "json_api plugin plugin_api"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CTP_APIKEY (api key), CTP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CTP_APIKEY carries the API key and
	// CTP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CTP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CTP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCtpSDK(opts)
