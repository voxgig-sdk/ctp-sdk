...
boru-driven command-line client **and** interactive REPL for the Ctp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ctp-cli)
make build
...
./ctp-cli load 1 json_api            # {id:1} shorthand
./ctp-cli load '{id:1}' json_api       # explicit match map

# 5. Override the API base URL for a single call
CTP_BASE=https://api.example.com ./ctp-cli load 1 json_api
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ctp-cli
   ```
...
```sh
./ctp-cli load 1 json_api          # scalar shorthand for {id:1}
./ctp-cli load '{id:1}' json_api     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ctp>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `json_api`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ctp-cli`. |
...
(`./ctp-cli <expr>`) and interactively (the REPL), and expressions compose the
