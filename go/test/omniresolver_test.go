...
import (
	...
	omni "github.com/voxgig-sdk/ctp-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CtpError); is {
				out := map[string]any{
					"name":    "CtpError",
...
func omniCtx(arg any, client *sdk.CtpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CtpSDK); is {
			client = live
