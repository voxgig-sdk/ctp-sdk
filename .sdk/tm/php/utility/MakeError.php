...
class CtpMakeError
{
    public static function call(?CtpContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CtpContext([], null);
        }
        $op = $ctx->op ?? new CtpOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CtpResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CtpError) ? $err->msg : (string)$err;
        $msg = "CtpSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new CtpError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof CtpError) {
