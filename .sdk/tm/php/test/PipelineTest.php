...
{
    private static function utility(): CtpUtility
    {
        return new CtpUtility();
    }
    ...
    private static function ctx(array $over = []): CtpContext
    {
        ...
        $ctx->op = new CtpOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CtpError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CtpOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CtpMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CtpOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CtpMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CtpOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CtpMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CtpSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CtpMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CtpResponse([]),
            'result' => new CtpResult([]),
        ]);
        [, $err] = CtpMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CtpSpec(['step' => 's']),
            'response' => new CtpResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CtpResult([]),
        ]);
        [, $err] = CtpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CtpSpec(['step' => 's']),
            'response' => new CtpResponse(self::resp(200, ['v' => 1])),
            'result' => new CtpResult([]),
        ]);
        [, $err] = CtpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CtpSpec(['step' => 's']),
            'response' => new CtpResponse(self::resp(200, ['v' => 2])),
            'result' => new CtpResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CtpMakeResponse::call($ctx);
    ...
    {
        $preset = new CtpResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CtpSpec([]),
            'response' => new CtpResponse([]),
            'result' => new CtpResult([]),
        ...
        [$got, $err] = CtpMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CtpResult([])]);
        [, $err] = CtpMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CtpSpec([]), 'result' => null]);
        [, $err] = CtpMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CtpSpec(['step' => 's']),
            'result' => new CtpResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CtpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CtpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CtpSpec(['step' => 's']),
            'result' => new CtpResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CtpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CtpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CtpResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CtpSpec([]),
            'result' => new CtpResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CtpMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new CtpError('boom', 'boom');
        $utility->fetcher = function (CtpContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CtpSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CtpMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CtpContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CtpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CtpResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CtpContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CtpSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CtpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CtpResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CtpSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CtpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CtpMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CtpResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CtpDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CtpResult(['ok' => false])]);
        $this->expectException(CtpError::class);
        CtpDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CtpResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CtpMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CtpBaseFeature();
        $b = new CtpBaseFeature();
        CtpFeatureAdd::call($ctx, $a);
        CtpFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CtpBaseFeature
    {
        $f = new CtpBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): CtpContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CtpSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new CtpResponse(['headers' => null]),
            'result' => new CtpResult([]),
        ]);
        CtpResultHeaders::call($ctx);
