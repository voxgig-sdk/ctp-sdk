...
class FtCtrl extends CtpControl
{
...
class FtClient
{
    ...
    private CtpUtility $utility;

    public function __construct(CtpUtility $utility, array $options)
    {
    ...
    public function get_utility(): CtpUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CtpContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CtpContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(CtpSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CtpError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CtpResponse($fetched);
        }
    ...
    private function fail(CtpContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CtpContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
