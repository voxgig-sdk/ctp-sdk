...
type Args struct {
	Entity string         `json:"entity" jsonschema:"json_api | plugin | plugin_api"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CtpSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "ctp_list",
		Description: "List records from Ctp. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "ctp_load",
		Description: "Load a single record from Ctp. " +
...
func runOp(client *sdk.CtpSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CtpSDK, name string) (sdk.CtpEntity, error) {
	switch strings.ToLower(name) {
	case "json_api":
		return client.JsonApi(nil), nil
	case "plugin":
		return client.Plugin(nil), nil
	case "plugin_api":
		return client.PluginApi(nil), nil
