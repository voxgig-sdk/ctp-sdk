...
MCP server exposing the Ctp SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `ctp_list` | `entity`, optional `query` map | First page of records as JSON |
| `ctp_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `ctp_list` and
`ctp_load` tools become available in new sessions.
