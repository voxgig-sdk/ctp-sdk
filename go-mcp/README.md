...
[MCP](https://modelcontextprotocol.io) server exposing the Ctp SDK as
two agent tools — `ctp_list` and `ctp_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ctp-mcp)
make build

# 2. Provide credentials via the environment
export CTP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ctp \
  -- /absolute/path/to/ctp-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ctp-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ctp_list: first page of records
{ "entity": "json_api" }
{ "entity": "json_api", "query": { } }

// ctp_load: one record by id
{ "entity": "json_api", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ctp-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ctp \
     -- "$PWD"/dist/*/ctp-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ctp_list` and `ctp_load` tools now appear
   in new sessions. Ask the agent to *"list json_api using ctp"*
   and it calls `ctp_list` with `{"entity":"json_api"}`.
...
```sh
export CTP_APIKEY=sk_live_xxx            # API key
export CTP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ctp_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ctp_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CTP_APIKEY` | API key sent with every request. |
| `CTP_BASE` | Optional override of the API base URL. |
...
```sh
./ctp-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ctp_load","arguments":{"entity":"json_api","query":{"id":1}}}}'
```
